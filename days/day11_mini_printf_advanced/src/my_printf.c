...
static int handle_format(char specifier, va_list args)
{
    ...
    if (specifier == 'u') {
        return my_put_unsigned_base(va_arg(args, unsigned int), "0123456789");
    }
    if (specifier == 'x') {
        return my_put_unsigned_base(va_arg(args, unsigned int), "0123456789abcdef");
    }
    if (specifier == 'X') {
        return my_put_unsigned_base(va_arg(args, unsigned int), "0123456789ABCDEF");
    }
    if (specifier == 'o') {
        return my_put_unsigned_base(va_arg(args, unsigned int), "01234567");
    }
    if (specifier == 'b') {
        return my_put_unsigned_base(va_arg(args, unsigned int), "01");
    }
    if (specifier == '%') {
        return my_putchar('%');
    }
